...
namespace TFNValidate.Services.Tests
{
    ...
    public class LinkedNumberCheckerTests
    {
        ...
        public void TestAreLinkedNumbersOverThreshold(int currentNumber, int[] previousNumbers, int maxLinkedCount, bool isOverThreshold)
        {
            var checker = new LinkedNumberChecker();
            Assert.That(checker.AreLinkedNumbersOverThreshold(currentNumber, previousNumbers, maxLinkedCount), Is.EqualTo(isOverThreshold));
        }
    }
}